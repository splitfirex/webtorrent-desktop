...
function renderMedia (state) {
  ...
  function playerClass () {
    let text =
      this.state.playing.subtitles.subtitlesColor === 0
        ? ' textColorWhite '
        : ' textColorYellow '
    text =
      text +
      (this.state.playing.subtitles.subtitlesBG === 0
        ? ' textBGnone '
        : this.state.playing.subtitles.subtitlesBG === 1
          ? ' textBGblack '
          : ' textBGTransparent ')

    text =
      text +
      (this.state.playing.subtitles.subtitlesSize === 0
        ? ' textS '
        : this.state.playing.subtitles.subtitlesSize === 1
          ? ' textM '
          : ' textL ')
    console.log(text)
    return text
...
function renderAudioMetadata (state) {
  ...
  if (fileSummary.audioInfo.format.bitrate) {
    format.push(
      Math.round(fileSummary.audioInfo.format.bitrate / 1000) + ' kbps'
    ) // 128 kbps
  }
...
function renderSubtitleControls (state) {
  ...
  return <SubtitlePage {...subtitleState} />
}
...
function renderPlayerControls (state) {
  ...
  elements.push(
    <div key='volume' className='volume float-left'>
      <i className='icon volume-icon float-left' onMouseDown={handleVolumeMute}>
        {volumeIcon}
      </i>
      <input
