...
module.exports = class TorrentListController {
  ...
  //Adds torrent deamon pooling
  poolingTorrent() {
    let poolingTime = this.state.saved.prefs.autoLoadTorrentPTime;
    let poolingUrl = this.state.saved.prefs.autoLoadTorrentJson;
    let poolingEnabled = this.state.saved.prefs.autoLoadTorrent;
    ...
    setTimeout(() => this.poolingTorrent(), poolingTime * 60000);
  }
  ...
  prioritizeTorrent(infoHash) {
    this.state.saved.torrents
      .filter((torrent) => {
        // We're interested in active torrents only.
      ...
      new electron.remote.MenuItem({
        ...
        click: () =>
          electron.clipboard.writeText(
            `https://instant.io/#${torrentSummary.infoHash}`
          ),
      })
    );
